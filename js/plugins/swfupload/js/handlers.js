...
function uploadSuccess(file, serverData) {
	try {
		...
		else if (serverData.substring(0, 6) === "IMAGE:") {
			addImage(serverData.substring(6), file.name);
		}
		else {
			addImage("swfupload/images/error.gif", file.name);
...
function uploadError(file, errorCode, message) {
	...
	try {
		...
		addImage("swfupload/images/" + imageName, file.name);
...
//	newTitle.style = "text-align: center; width: auto;";

	var divThumbs = document.getElementById("thumbnails");
//	divThumbs.insertBefore(newTitle, divThumbs.firstChild);
